...
def compute_ccas(sigma_xx, sigma_xy, sigma_yx, sigma_yy, epsilon,
  ...
  if numx == 0 or numy == 0:
    return ([0, 0, 0], [0, 0, 0], [0, 0, 0]), np.zeros_like(sigma_xx), \
...
def get_cca_similarity(acts1, acts2, epsilon=0., threshold=0.98,
  ...
  print(compute_ccas(sigmaxx, sigmaxy, sigmayx, sigmayy,
                                  epsilon=epsilon,
                                  verbose=verbose))
  ([u, s, v]), invsqrt_xx, invsqrt_yy, x_idxs, y_idxs = compute_ccas(sigmaxx, sigmaxy, sigmayx, sigmayy,
                                  epsilon=epsilon,
                                  verbose=verbose)
